...
Neither source replaces the other. The video gives the visual timeline; the behavioral source gives the behavioral record.
...
For example, a risk event can produce a clip that starts when the noise LED turns on, while its MATLAB lever latency starts a few seconds later when the food light turns on. Both times are valid; they describe different parts of the same event.
...
- a timeout;
- a sound-only control event (in new sessions).

Historical files have eight columns. New CP or DIS files with sound-only events have a ninth column, `TipoEvento`:
...
The reader must preserve this information; it must never modify or rename the source MAT or CSV.

## Why Synchronization Is Approximate

The video and MATLAB do not start from a guaranteed shared clock. There can be recording delay, MATLAB timing delay, and manually determined habituation boundaries.

Therefore the goal is **not** to force every behavioral timestamp onto one exact video frame. The goal is to find the best supported association and make any uncertainty visible to the reviewer.
...
For a normal risk event, the output clip may start at `Noise LED ON`. The food light ON/OFF frames remain visual references that are compared with MATLAB’s event start estimate and lever-press time; they are not assumed identical.

The detailed measurement, offset-estimation, and validation rules are defined in [CajaValentia Video-Behavior Synchronization](../project/sincronizacion-video-mat-cajavalentia.md).
In particular, it derives the MATLAB event-start estimate as `TiempoAbs - Latencia`, measures both the start gap and the post-press light tail, and estimates their session patterns from multiple matched events rather than assuming a fixed delay.
...
VideoReader -> FrameAnalyzer -> BrightnessAdapter -> LightDetection -> LightTimelineBuilder
 ...
 MAT  --> BehavioralSessionReader -> BehavioralEvent[] ---> SegmentPlanner -> VideoSegment
```
...
- `SegmentPlanner` pairs them, proposes clip boundaries, and produces warnings when the association is doubtful.

## Practical Matching Rules

When this functionality is implemented, use these rules:

1. Confirm that the video and behavioral source belong to the same session using their metadata and file relationship.
2. Detect stable food-light and noise-LED transitions from the video.
3. Read behavioral rows in recorded order.
4. Match visual events to behavioral events in order, using repeated light events and timestamps as evidence rather than assuming perfect equality.
...
from CSV V1 or historical `.mat` files. If Excel is present later, it can be compared as an extra check. A disagreement between Excel, behavioral data, and video is a warning to report; the video remains the visual evidence and the behavioral source remains the behavioral record.
