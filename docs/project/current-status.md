...
| Procesamiento por lote CS | Integrado en interfaz; validado con una sesión completa | `Procesar sesiones` toma la cámara y calibración confirmadas de una sesión de referencia y puede procesar uno o muchos videos de la lista visible. Antes de escanear, si existen resultados, permite cancelar, omitir esas sesiones o archivarlas con fecha y crear una corrida nueva; nunca borra automáticamente. El lote toma solo videos CS fuente, empareja el MAT exacto, escanea luces, sincroniza, guarda automáticamente un Excel diagnóstico y exporta clips con FFmpeg. Cada carpeta de salida se crea junto a su video fuente e incluye `clips_exportados.csv`: distingue el rango lógico del evento del rango físico exportado y conserva el ensayo original. Los nombres `crN` y `ncN` cuentan cruces y no cruces por separado. Los eventos reciben cinco frames de contexto antes y después; habituación e ITIs no. Bloquea sesiones sin MAT, metadata suficiente o sincronización válida. |
...
El estado actual compila y tiene **180 pruebas** aprobadas. La interfaz también
compila con Avalonia 12 y mantiene esas 180 pruebas.
