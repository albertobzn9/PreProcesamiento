...
| Procesamiento por lote CS | Integrado en interfaz; pendiente de validación real | `Process Batch` toma la cámara y calibración confirmadas de una sesión de referencia, usa la carpeta cargada como salida y llama a `BatchOrchestrator`. Primero ofrece una prueba de límites con pocos clips por sesión; después permite exportar todo. El lote toma solo videos CS fuente, empareja el MAT exacto, escanea luces, sincroniza, guarda un Excel diagnóstico y exporta clips con FFmpeg. Cada carpeta de salida incluye `clips_exportados.csv` con sus frames y tiempos de inicio/final en el video original. Bloquea sesiones sin MAT, metadata suficiente o sincronización válida; no sobrescribe una salida existente. |
...
El estado actual compila y tiene **174 pruebas** aprobadas. La interfaz también
compila con Avalonia 12 y mantiene esas 174 pruebas.
